...
async def play_the_game(app_token, promo_id):
    ...
    try:
        client_token = await login(client_id, app_token)
         ...
         # This will catch HTTP errors and you can access the response
         logger.error(f"Failed to login: {e.response.json()} id: {client_id}")
         return None
    except httpx.RequestError as e:
        # This will catch all other request-related errors like ProxyError, Timeout, etc.
        logger.error(f"Failed to login due to a network issue: {str(e)} id: {client_id}")
        return None
